...
describe('POST /api/sources/import', () => {
  it('creates a source per feed and a tag per folder', async () => {
    const res = await agent.post('/api/sources/import').send({ opml: OPML });
  ...
  it('attaches the folder tags to the right sources', async () => {
    await agent.post('/api/sources/import').send({ opml: OPML });

    const sources = await agent.get('/api/sources?q=Nutanix');
  ...
  it('re-importing the same file is a no-op, not an error', async () => {
    await agent.post('/api/sources/import').send({ opml: OPML });
    const second = await agent.post('/api/sources/import').send({ opml: OPML });

    expect(second.status).toBe(201);
    expect(second.body.data).toMatchObject({ created: 0, alreadyTracked: 5 });
    expect((await agent.get('/api/sources')).body.data).toHaveLength(5);
  });

  it('applies extra tags on top of the folder tags', async () => {
    const imported = await agent.post('/api/tags').send({ name: 'Imported' });

    await agent.post('/api/sources/import').send({ opml: OPML, tagIds: [imported.body.data.id] });

    const sources = await agent.get(`/api/sources?tag=${imported.body.data.id}`);
    expect(sources.body.data).toHaveLength(5);
  });

  it('can skip folder tags entirely', async () => {
    await agent.post('/api/sources/import').send({ opml: OPML, importCategoriesAsTags: false });

    expect((await agent.get('/api/tags')).body.data).toEqual([]);
    expect((await agent.get('/api/sources')).body.data).toHaveLength(5);
  });
  ...
  it('rejects unknown tag ids', async () => {
    const res = await agent.post('/api/sources/import').send({ opml: OPML, tagIds: [9999] });
...
describe('GET /api/sources/export', () => {
  it('round-trips an import through an export', async () => {
    ...
    const exported = await agent.get('/api/sources/export');

    expect(exported.status).toBe(200);
    expect(exported.headers['content-type']).toContain('opml');
    expect(exported.headers['content-disposition']).toContain('nexuscentral-sources.opml');

    // Re-import into an empty database and the same five sources come back with
    // the same tags.
    await resetDatabase();
    const reimported = await agent.post('/api/sources/import').send({ opml: exported.text });

    expect(reimported.body.data.created).toBe(5);

    const nutanix = await agent.get('/api/sources?q=Nutanix');
  ...
  it('reports how many sources have no OPML representation', async () => {
    ...
    expect(exported.headers['x-nexuscentral-skipped-sources']).toBe('1');
