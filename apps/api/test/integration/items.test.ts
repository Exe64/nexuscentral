...
async function seed(): Promise<Seeded> {
  ...
  const network = await request(app).post('/api/tags').send({ name: 'Networking' });
...
describe('GET /api/items', () => {
  ...
  it('sorts by score and by engagement', async () => {
    const byScore = await request(app).get('/api/items?sort=score&limit=2');
    expect(byScore.body.data.map((i: { score: number }) => i.score)).toEqual([8.4, 6.5]);

    const byEngagement = await request(app).get('/api/items?sort=engagement&limit=2');
    expect(
  ...
  it('puts items with no engagement signal last rather than dropping them', async () => {
    const res = await request(app).get('/api/items?sort=engagement');
    expect(res.body.data).toHaveLength(6);
    expect(res.body.data.at(-1).engagementScore).toBeNull();
  });

  it('filters by tag as a union, not an intersection', async () => {
    const oneTag = await request(app).get(`/api/items?tagIds=${seeded.storageTagId}`);
    expect(oneTag.body.data).toHaveLength(3);

    const bothTags = await request(app).get(
  ...
  it('filters by source, star, score floor and date', async () => {
    expect(
      (await request(app).get(`/api/items?sourceIds=${seeded.betaId}`)).body.data,
    ).toHaveLength(3);
    expect((await request(app).get('/api/items?starredOnly=true')).body.data).toHaveLength(1);
    expect((await request(app).get('/api/items?minScore=6')).body.data).toHaveLength(2);

    const since = new Date(Date.now() - 6 * 3600 * 1000).toISOString();
    expect((await request(app).get(`/api/items?since=${since}`)).body.data).toHaveLength(3);
  });

  it('runs full-text search over the title and the summary', async () => {
    const byTitle = await request(app).get('/api/items?q=kubernetes');
    expect(byTitle.body.data.map((i: { title: string }) => i.title)).toEqual([
      'Kubernetes 1.35 deprecations',
    ]);

    const bySummary = await request(app).get('/api/items?q=benchmarks');
...
describe('cursor pagination', () => {
  ...
  it('paginates a score sort without losing tied rows', async () => {
    ...
    const first = await request(app).get('/api/items?sort=score&limit=4');
...
describe('read and star state', () => {
  it('marks an item read and unread', async () => {
    ...
    expect((await request(app).post(`/api/items/${id}/read`)).status).toBe(204);
    expect((await request(app).get(`/api/items/${id}`)).body.data.readAt).not.toBeNull();

    expect((await request(app).delete(`/api/items/${id}/read`)).status).toBe(204);
    expect((await request(app).get(`/api/items/${id}`)).body.data.readAt).toBeNull();
  });
  ...
  it('stars and unstars an item', async () => {
    ...
    await request(app).post(`/api/items/${id}/star`);
    expect((await request(app).get(`/api/items/${id}`)).body.data.starred).toBe(true);

    await request(app).delete(`/api/items/${id}/star`);
    expect((await request(app).get(`/api/items/${id}`)).body.data.starred).toBe(false);
  });

  it('404s for an unknown item and 400s for a non-numeric id', async () => {
    expect((await request(app).post('/api/items/999999/read')).status).toBe(404);
    expect((await request(app).get('/api/items/abc')).status).toBe(400);
  });

  it('excludes read items when unreadOnly is set', async () => {
    const items = await request(app).get('/api/items');
    await request(app).post(`/api/items/${items.body.data[0].id}/read`);

    const unread = await request(app).get('/api/items?unreadOnly=true');
...
describe('POST /api/items/read-all', () => {
  it('marks only the items matching the given filters', async () => {
    const res = await request(app)
      .post('/api/items/read-all')
      .send({ sourceIds: [seeded.betaId] });
  ...
  it('is idempotent — a second call updates nothing', async () => {
    await request(app).post('/api/items/read-all').send({});
    const second = await request(app).post('/api/items/read-all').send({});
    expect(second.body.data.updated).toBe(0);
  });

  it('honours a full-text filter', async () => {
    const res = await request(app).post('/api/items/read-all').send({ q: 'kubernetes' });
